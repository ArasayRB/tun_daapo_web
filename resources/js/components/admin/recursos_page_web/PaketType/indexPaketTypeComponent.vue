<template>
  ...
  <div class="row py-lg-2">
    <div class="col-md-6">
      <h1 class="h3 mb-2 text-gray-800">{{ $trans('messages.Paket Type') }}</h1>
    </div>
    <div class="col-md-6">
      ...
      console.log(evt);
        ...
        deletePaketType:function(index,paket_type){
          let paket_type_id=paket_type;
            swal({title:this.$trans('messages.Delete')+' '+this.$trans('messages.Paket Type'),
                  text:this.$trans('messages.Are you completely sure you want to delete ')+this.$trans('messages.Paket Type')+'?',
                  icon:'warning',
                    ...
                    axios.delete(url)
                         .then(response=>{
                           swal({title:this.$trans('messages.Correct data'),
                                 text:this.$trans('messages.Paket Type')+' '+this.$trans('messages.Deleted'),
                                 icon:'success',
                                 closeOnClickOutside:false,
                                 closeOnEsc:false
                               }).then(select=>{
